...
# COMBINE EINSATZDATEN (2018-2022 and 2023)
# Common date columns
DATE_COLUMNS = [
...
def load_raw_einsatzdaten(file_path, 
    ...
    df = pd.read_excel(
        ...
        sheet_name=0,
        engine="openpyxl",
        parse_dates=False   # read raw, no parsing yet
    )

    # now parse with dayfirst
...
def combine_einsatzdaten():
    ...
    combined = pd.concat([df_2024, ], ignore_index=True)
...
# ADD RESSOURCENSTANDORT COORDINATES (fz_lat/fz_lon)
...
# CALCULATE ORS DISTANCE & DURATION, SAVE INTERMEDIATE
...
# FINAL TRANSFORMATIONS & FILTERING
...
def final_filter_and_save():
    ...
    columns_to_drop = [
        ...
        "EINSATZSTICHWORT","EINSATZORT_X","EINSATZORT_Y","ZIELORT","ZIELORT_X","ZIELORT_Y",
    ...
    df.drop(columns=['Einsatzniveau','SONDERSIGNAL'], errors="ignore", inplace=True)
...
df = pd.read_parquet("../data/interim/combined_einsatz_ORS_2024.parquet")
df.info()

# 4) Merge with weather
merge_with_weather()

df = pd.read_parquet("../data/interim/combined_einsatz_weather_2024.parquet")
df.info()
