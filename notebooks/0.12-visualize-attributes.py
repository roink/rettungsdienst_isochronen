...
file_path = os.path.join(directory, "einsatz_data.parquet")
data= pd.read_parquet(file_path)

import matplotlib.pyplot as plt

# Drop missing values in 'MELDUNGSEINGANG' if any
meldungen_data = data['MELDUNGSEINGANG'].dropna()

# Plot the histogram with daily bins
plt.figure(figsize=(10, 6))
plt.hist(meldungen_data, bins=30, color='skyblue', edgecolor='black')  # Adjust bins as needed
plt.title("Distribution of 'MELDUNGSEINGANG' Datetimes")
plt.xlabel("Date")
...
plt.figure(figsize=(10, 6))
plt.hist(alarmzeit_hours, bins=24, range=(0, 24), color='salmon', edgecolor='black')
plt.title("Distribution of 'ALARMZEIT' by Time of Day")
...
# Drop missing values and extract the hour for both 'ALARMZEIT' and 'MELDUNGSEINGANG'
alarmzeit_hours = data['ALARMZEIT'].dropna().dt.hour
meldungseingang_hours = data['MELDUNGSEINGANG'].dropna().dt.hour

# Plot the histograms for both 'ALARMZEIT' and 'MELDUNGSEINGANG' on the same plot
plt.figure(figsize=(10, 6))
plt.hist(meldungseingang_hours, bins=24, range=(0, 24), color='skyblue', alpha=0.6, edgecolor='black', label="MELDUNGSEINGANG")
...
plt.title("Distribution of 'ALARMZEIT' by Day of the Week")
...
# Count values for "J" (Yes), "N" (No), and NaN (missing values)
sondersignal_counts = data['SONDERSIGNAL'].value_counts(dropna=False)
labels = ['Yes', 'No', 'NA']  # Custom labels for clarity
sizes = [sondersignal_counts.get(True, 0), sondersignal_counts.get(False, 0), sondersignal_counts.isna().sum()]

# Plot a pie chart
plt.figure(figsize=(8, 6))
plt.pie(sizes, labels=labels, autopct='%1.1f%%', startangle=90, colors=['lightgreen', 'lightcoral', 'lightgray'])
plt.title("Distribution of 'SONDERSIGNAL' (Yes, No, NA)")
plt.show()
