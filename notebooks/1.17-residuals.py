...
# ------------------------------
df = pd.read_parquet("../data/interim/selected-data.parquet")
...
print("[LOG] Performing 5-fold cross-validation to get out-of-fold predictions...")
cv = KFold(n_splits=5, shuffle=True, random_state=42)

# cross_val_predict klont den Pipeline-Estimator intern und passt ihn in jedem Fold neu an
y_pred = cross_val_predict(clone(pipeline_model), X, y, cv=cv, n_jobs=30)

# Residuen auf den jeweils ausgelassenen Daten
residuals = y - y_pred
...
plt.ylim(-15, 15)
plt.tight_layout()
plot_output_path = "../reports/figures/1.17-Residuen-Vorhersage.png"
...
plt.xlabel("Tatsächliche Hilfsfrist [min]")
plt.ylabel("Residuum [min]")
plt.title("Residuen in Abhängigkeit von der Hilfsfrist")
plt.xlim(x_vals.min() - 1, x_vals.max() + 1)
plt.ylim(-15,15)
plt.tight_layout()
plt.savefig("../reports/figures/1.17-Residuen-Boxplot.png", dpi=300)
plt.show()
...
plot_output_path = "../reports/figures/1.17-Residuen-Verteilung.png"
plt.savefig(plot_output_path, dpi=300)
plt.show()
...
fig.tight_layout()
fig.savefig("../reports/figures/1.17-Residuen-QQ.png", dpi=300)
plt.show()
